...
class ElectronicsAgent(ResponsesAgent):
    """
    A LangGraph agent designed to assist users in finding products from a catalog using a combination
    of retrieval and LLM capabilities. Primarily leverages UC Functions for querying and product
    information.

    Attributes:
        agent (LangGraph Agent): The LangGraph agent instance. Initialized as None and loaded in `load_context`.
    """
    def __init__(self):
        """
        Initializes an ElectronicsAgent instance.
        """
        self.agent = None
    ...
    def load_context(self, context=None):
        ...
        Args:
            context (Optional[dict]): A dictionary containing configuration parameters, including the LLM endpoint,
                                       temperature, and function names.  If None, uses default values.
        """
        #auto log
        ...
        system_instructions = (
            ...
            "Always return a response. If you can't find anything, say so."
            "Your primary goal is to help users find the right products from our catalog."
            "But if you can't find anything from the catalog, get it from the foundational model but say that and provide references.\n\n"
            "GUIDELINES:\n"
            ...
            "5. Do not hallucinate product names or features not found in the catalog or from the foundational model"
        )
    ...
    def predict(self, request: ResponsesAgentRequest) -> ResponsesAgentResponse:
        """
        Generates a response to a user request using the loaded LangGraph agent.

        Args:
            request (ResponsesAgentRequest): The user request, including input messages and optional context.

        Returns:
            ResponsesAgentResponse: The agent's response, formatted as a list of messages.
        """
