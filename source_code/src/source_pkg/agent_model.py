...
class ElectronicsAgent(ResponsesAgent):
    ...
    def predict(self, request: ResponsesAgentRequest) -> ResponsesAgentResponse:
        ...
        thread_id = getattr(context, "conversation_id", str(uuid.uuid4())) if context else str(uuid.uuid4())
        agent_input = {"messages": [m.model_dump() for m in request.input]}
        ...
        return ResponsesAgentResponse(
            output=[{
                ...
                "role": "assistant",
                "id": str(uuid.uuid4()),
                "content": [{"type": "output_text", "text": result["messages"][-1].content}]
            }]
        )

mlflow.models.set_model(ElectronicsAgent())